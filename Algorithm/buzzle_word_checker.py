import re
import PyPDF2
import docx
import tkinter as tk
from tkinter import filedialog, scrolledtext

# List of predefined buzzwords
buzzwords = [
    "Leadership", "Team Management", "Conflict Resolution",
    "Communication", "Presentation Skills", "Public Speaking",
    "Problem Solving", "Critical Thinking", "Analytical Skills",
    "Collaboration", "Teamwork", "Cross-functional Collaboration",
    "Time Management", "Organization", "Multitasking",
    "Adaptability", "Flexibility", "Decision Making",
    "Creativity", "Innovation", "Design Thinking",
    "Negotiation", "Client Relations", "Customer Support",
    "Relationship Building", "Networking", "Stakeholder Management",
]

def extract_text_from_pdf(pdf_path):
    """Extracts text from a PDF file."""
    text = ""
    with open(pdf_path, "rb") as file:
        pdf_reader = PyPDF2.PdfReader(file)
        for page in pdf_reader.pages:
            text += page.extract_text() + "\n"
    return text

def extract_text_from_docx(docx_path):
    """Extracts text from a Word file."""
    doc = docx.Document(docx_path)
    text = "\n".join([para.text for para in doc.paragraphs])
    return text

def extract_buzzwords(text):
    """Extracts buzzwords from the text."""
    found_buzzwords = []
    for buzzword in buzzwords:
        if re.search(r'\b' + re.escape(buzzword) + r'\b', text, re.IGNORECASE):  # Case insensitive search
            found_buzzwords.append(buzzword)
    return found_buzzwords

def open_file():
    """Opens a file dialog to upload a resume and process it."""
    file_path = filedialog.askopenfilename()

    if not file_path:
        return  # If no file is selected, do nothing

    # Check if the file format is valid
    if not (file_path.endswith(".pdf") or file_path.endswith(".docx")):
        result_box.config(state=tk.NORMAL)
        result_box.delete("1.0", tk.END)
        result_box.insert(tk.END, "Invalid format! Please upload a PDF or Word file.\n")
        result_box.config(state=tk.DISABLED)
        return  # Stop further execution if the format is invalid

    # Extract text based on file type
    if file_path.endswith(".pdf"):
        extracted_text = extract_text_from_pdf(file_path)
    elif file_path.endswith(".docx"):
        extracted_text = extract_text_from_docx(file_path)

    # Extract buzzwords
    found_buzzwords = extract_buzzwords(extracted_text)

    # Display results in GUI
    result_box.config(state=tk.NORMAL)  # Enable editing
    result_box.delete("1.0", tk.END)  # Clear previous text

    if found_buzzwords:
        result_box.insert(tk.END, "=== Buzzwords Found in Resume ===\n", "bold")
        for buzzword in found_buzzwords:
            result_box.insert(tk.END, f"{buzzword}\n")
    else:
        result_box.insert(tk.END, "No buzzwords found in the resume.\n")

    result_box.config(state=tk.DISABLED)  # Disable editing

# GUI Setup
root = tk.Tk()
root.title("Resume Buzzword Extractor")

upload_button = tk.Button(root, text="Upload Resume", command=open_file)
upload_button.pack(pady=10)

result_box = scrolledtext.ScrolledText(root, wrap=tk.WORD, width=80, height=30)
result_box.pack(padx=10, pady=10)
result_box.tag_config("bold", font=("Arial", 10, "bold"))
result_box.config(state=tk.DISABLED)

root.mainloop()